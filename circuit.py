...
def DiffuserGrover(n):
    ...
    diffuser_gate = qc.to_gate(label = "Diffuser")
    return diffuser_gate

def Overall_SAT_Circuit(input_qubits, constraints, k = 1):
    '''
        Functionality:
        ...
        Parameters:
            ...
            k (int default = 1) - the excpected amount of solutions for the SAT problem.
            # TODO - neeed to remove the `k` method and replace it with a method for solving SAT without former knowledge of k.
        Returns:
    ...
    iterations = int((np.pi / 4) * np.sqrt(N / k)) # Iterations over Grover's iterator (= over operator + diffuser)
